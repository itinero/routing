...
namespace OsmSharp.Routing.Test.Data.Contracted
{
    ...
    public class ContractedEdgeDataSerializerTests
    {
        ...
        public void TestSerialize()
        {
            ...
            Assert.AreEqual(((uint)(100.0f * 4)), data[0]);
            ...
            Assert.IsNotNull(data);
            Assert.AreEqual(2, data.Length);
            Assert.AreEqual(((uint)1) + (((uint)(ContractedEdgeDataSerializer.MAX_DISTANCE * 4))), data[0]);
            Assert.AreEqual(Constants.NO_VERTEX, data[1]);
            ...
            Assert.IsNotNull(data);
            Assert.AreEqual(2, data.Length);
            Assert.AreEqual(((uint)2) + (((uint)(ContractedEdgeDataSerializer.MAX_DISTANCE * 4))), data[0]);
            Assert.AreEqual(Constants.NO_VERTEX, data[1]);
        ...
        public void TestDeserialize()
        {
            ...
            edge = ContractedEdgeDataSerializer.Deserialize(new uint[] { ((uint)1 + (((uint)(ContractedEdgeDataSerializer.MAX_DISTANCE * 4)))),
                Constants.NO_VERTEX });
            Assert.AreEqual(true, edge.Direction);
            Assert.AreEqual(ContractedEdgeDataSerializer.MAX_DISTANCE, edge.Weight);
            Assert.AreEqual(Constants.NO_VERTEX, edge.ContractedId);

            edge = ContractedEdgeDataSerializer.Deserialize(new uint[] { ((uint)2 + (((uint)(ContractedEdgeDataSerializer.MAX_DISTANCE * 4)))),
                Constants.NO_VERTEX });
            Assert.AreEqual(false, edge.Direction);
            Assert.AreEqual(ContractedEdgeDataSerializer.MAX_DISTANCE, edge.Weight);
            Assert.AreEqual(Constants.NO_VERTEX, edge.ContractedId);
